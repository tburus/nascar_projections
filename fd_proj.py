...
PURPOSE: function for creating FD projection; imported to fantasy_proj.py
'''
import pandas as pd
import numpy as np
...
def fd_proj(outfile, laps, entry_file, tr1, tr2, tr3, tr4, tr5, tr6, tr7, tr8):
    ...
    fantasy_stats = ['races', 'avg_start', 'avg_finish', 'avg_pd', 'fl', 'll', 'lc', 'tl']
    ...
    rec_stats['percent_lc'].fillna(0, inplace=True)
    ...
    rec_stats['lc_rank'] = rec_stats['percent_lc'].rank(ascending=False)
    ...
    #compute points in FanDuel
    rec_stats['pd_pts'] = rec_stats['avg_pd'] * 0.5
    rec_stats['ll_pts'] = rec_stats['percent_ll'] * int(laps) * 0.1
    rec_stats['lc_pts'] = rec_stats['percent_lc'] * int(laps) * 0.1
    rec_stats['finish_pts'] = (44 - rec_stats['finish_rank'])
    ...
    rec_stats.to_csv('NASCAR_' + year + '\\' + outfile + '\\' + 
                     'fd_fantasy_' + outfile + '.csv', index = False)
